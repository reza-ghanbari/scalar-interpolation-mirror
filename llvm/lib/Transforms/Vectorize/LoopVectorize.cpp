...
protected:
  ...
  /// Checks if the instruction is a temp.add generated by scalar interpolation
  bool isScalarInterpolationAuxiliaryInstruction(User *U, VPlan &Plan);
  ...
  void fixInterpolatedPhi(VPInterpolatePHIRecipe *Phi, VPTransformState &State);
...
void InnerLoopVectorizer::emitIterationCountCheck(BasicBlock *Bypass) {
  ...
  auto CreateStep = [&]() -> Value * {
    ...
    if ((UF * VF.getKnownMinValue() + ScalarInterpolationFactor) >= MinProfitableTripCount.getKnownMinValue())
      return createStepForVF(Builder, CountTy, VF, UF, ScalarInterpolationFactor);

    Value *MinProfTC =
        createStepForVF(Builder, CountTy, MinProfitableTripCount, 1, ScalarInterpolationFactor);
    if (!VF.isScalable())
      return MinProfTC;
    return Builder.CreateBinaryIntrinsic(
        Intrinsic::umax, MinProfTC, createStepForVF(Builder, CountTy, VF, UF, ScalarInterpolationFactor));
  };
  ...
  else if (VF.isScalable() && !isIndvarOverflowCheckKnownFalse(Cost, VF, UF) &&
           Style != TailFoldingStyle::DataAndControlFlowWithoutRuntimeCheck) {
...
void InnerLoopVectorizer::fixupIVUsers(PHINode *OrigPhi,
  ...
  for (User *U : OrigPhi->users()) {
    ...
    if (!OrigLoop->contains(UI) && !isScalarInterpolationAuxiliaryInstruction(U, Plan)) {
      assert(isa<PHINode>(UI) && "Expected LCSSA form");
...
bool InnerLoopVectorizer::isScalarInterpolationAuxiliaryInstruction(User *U, VPlan &Plan) {
  if (ScalarInterpolationFactor > 0 && Plan.isInterpolatedValue(U))
    return true;
  auto *I = cast<Instruction>(U);
  auto* AnnotationNode = I->getMetadata(LLVMContext::MD_annotation);
  if (!AnnotationNode)
    return false;
...
void InnerLoopVectorizer::fixInterpolatedPhi(VPInterpolatePHIRecipe *PhiR,
                                       VPTransformState &State) {
  auto* UnderlyingReduction = dyn_cast<VPReductionPHIRecipe>(PhiR->getUnderlyingRecipe());
...
void InnerLoopVectorizer::sinkScalarOperands(Instruction *PredInst) {
  ...
  do {
    ...
    while (!Worklist.empty()) {
      ...
      // TODO Could dor more granular checking to allow sinking a load past
...
LoopVectorizationCostModel::getDivRemSpeculationCost(Instruction *I,
  ...
  if (!VF.isScalable()) {
    ...
    ScalarizationCost +=
        VF.getKnownMinValue() * TTI.getCFInstrCost(Instruction::PHI, CostKind);
  ...
  SafeDivisorCost += TTI.getArithmeticInstrCost(
      ...
      {TargetTransformInfo::OK_AnyValue, TargetTransformInfo::OP_None}, Op2Info,
      Operands, I);
...
static void addCanonicalIVRecipes(VPlan &Plan, Type *IdxTy, DebugLoc DL,
  ...
  if (useActiveLaneMaskForControlFlow(Style)) {
    ...
    if (Style == TailFoldingStyle::DataAndControlFlowWithoutRuntimeCheck) {
      ...
      //      TODO-SI: check if you need to add SI into this calculation or not
      auto *TCMinusVF =
...
SmallVector<llvm::VPInterpolatePHIRecipe*> VPTransformState::getAllSimilarInterpolateRecipes(VPInterpolatePHIRecipe* OriginalRecipe) {
    SmallVector<llvm::VPInterpolatePHIRecipe*> SimilarRecipes;
